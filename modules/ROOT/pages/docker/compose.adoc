...
IMPORTANT: Create a directory `~/.nom/ssc` before running Docker compose. Certificates will be generated as by products of running below compose file at `~/.nom/ssc` directory which can be used to configure the NOM agent as given xref:addition/index.adoc#configure[here].
