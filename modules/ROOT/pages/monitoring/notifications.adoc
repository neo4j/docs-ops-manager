...
+
Email notifications can be sent to email addresses of NOM users as well as to external email addresses label:new[Introduced in 1.14]. Enter an external email address in the "Notify to" field and confirm with "Create".
Previously used external email addresses are saved and can be selected from the dropdown.
