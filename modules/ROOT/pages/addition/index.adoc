...
[IMPORTANT]
====
`n` in the above environment variables needs to be replaced with `1`, `2`, etc. for each of the monitored DBMS instance(s) on the same host.
...
[IMPORTANT]
====
The instance name that you specify for `CONFIG_INSTANCE_n_NAME` will be used to identify your instance on NOM user interface.
