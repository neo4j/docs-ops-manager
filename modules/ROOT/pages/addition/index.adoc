...
The following tables list configuration keys and values to be set for them.

The following environement variables need to be set to allow the agent to communicate with the NOM Server.
...
The following environement variables specify start configuration for the agent:
[cols="<,<,<",options="header"]
|===
| Variable
| Description
...
[NOTE]
====
Refer to *xref:../addition/instance-requirements[Neo4j instance requirements]* to ensure that all instances meet the rquirements to be managed by NOM.
====
