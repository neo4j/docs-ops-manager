...
Instead minimal configuration about the NOM server is provided to allow agent to connect and register itself.
Authorization is granted either through manual approval of the agent in UI or through mutual authentication.
...
NOM server will immediately authorize an agent that registers using a trusted certficate.
...
IMPORTANT: *In addition to the above configuration, NOM server also needs to be configured to trust the agent certificates as described xref:/installation/server.adoc#config_ref[here]*
...
To enable the agent to continue its normal execution, agent needs to be approved from NOM UI as shown below:
