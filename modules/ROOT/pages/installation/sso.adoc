...
| Username/password login can be enabled or disabled, enabling SSO login either be offered additionally or to be the only login method. If disabled, the pages "NOM Administrators" and "Profile" will not be available in the NOM UI.
| `false`
...
Enable debug logging by setting the NOM server config option `logging.level.com.neo4j.opsmanager.server.config` to `debug`.
