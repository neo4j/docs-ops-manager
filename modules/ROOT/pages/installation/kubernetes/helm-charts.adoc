...
* Add the NOM Helm chart repository.
[source, shell]
----
...
[source, shell]
----
helm repo update nom
----
* Check available versions of NOM charts.
[source, shell]
-----
helm search repo nom/ --versions
-----
* Install the chart from repo:
[source, shell]
----
helm install my-nom-server nom/neo4j-ops-manager-server -f values.yaml
----
* Alternatively, you can download the NOM server Helm Chart from link:https://neo4j.com/deployment-center/?ops-manager[Neo4j Deployment Center].
...
NOM server now provides Single Sign-on support for select few Identity providers (Refer to xref:../installation/sso/configuration.adoc[SSO Configuration]).
To simplify configuring NOM server in the chart, following values can be set in `values.yaml` under `ssoConfigs` to be translated to NOM server SSO configuration:

NOTE: Each SSO configuration is added as a key in `ssoConfigs` map referred here as `<CONFIG>`.
...
NOTE: The default username & password login can be disabled by setting `config.usernamePasswordLoginEnabled` to `false`.
