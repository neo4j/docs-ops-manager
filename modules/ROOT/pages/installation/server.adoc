...
Default database for a user is either the home database for that user if it is set or the default database for the DBMS. 
The default database for the DBMS will be 'neo4j' if not explicitly configured.
====
