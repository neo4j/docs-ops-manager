...
* It is recommended to create 2 user roles to be mapped to NOM’s Admin and Read-only user roles to keep NOM server configuration simple. 
If your Auth0 tenant has complex user roles that need to be mapped to NOM roles, please use the SpEL expressions as described xref:./configuration.adoc#_role_assignment[here].
...
Otherwise the shell may interpret them, but it depends on which shell you are using.
====
