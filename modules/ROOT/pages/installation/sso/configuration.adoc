...
The name can be chosen freely, but *must* consist only of alphanumeric characters.
...
[NOTE]
====
Claim field values for email address, first and last names default to an empty string if they evaluate to `null`, resulting in a warning being logged.
It is recommended using non-empty claim field values for better user experience.
Username claim field evaluation must result in a non-blank string otherwise authentication will be aborted with an error.
====
