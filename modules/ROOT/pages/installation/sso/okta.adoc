...
* Configure the new app integration with the following:
    ...
    ** Add `https://<NOM_SERVER_HOSTNAME>:<NOM_SERVER_PORT>/okta-callback` (omit the port if using default 443) as the only `Sign-in redirect URIs`.
    ** Add `https://<NOM_SERVER_HOSTNAME>:<NOM_SERVER_PORT>` as the only `Sign-out redirect URIs`.
...
* We recommend to create/assign 2 user `Groups` to be mapped to NOM's Admin & User roles to keep the NOM server configuration simple.
