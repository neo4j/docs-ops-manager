...
services:
  ...
  server:
    ...
    entrypoint:
      - "sh"
      - "-c"
      - "java -jar app.jar ssc -n server -o /certificates -p changeit -d server -i 127.0.0.1 && java -jar app.jar"
----

Docuemntation for NOM server docker image is *xref:installation/docker/server.adoc[here]*.
  ...
  db-single:
    ...
    volumes:
       - type: bind
          ...
          target: /certificates
    healthcheck:
      test: [ "CMD-SHELL", "echo RETURN 1 | cypher-shell -a bolt://db-single:10001 -u neo4j -p passw0rd || exit 1" ]
      interval: 10s
      timeout: 10s
      retries: 3
      start_period: 5s
