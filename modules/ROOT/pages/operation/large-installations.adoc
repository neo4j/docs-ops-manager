...
| `0 05 * * * *`
| Cron expression that controls how often the aggregation runs.
...
[NOTE]
====
In some versions of Neo4j Ops Manager, changing `metrics.aggregation.schedule` will take effect, but the corresponding log line might still refer to it as `Running scheduled hourly metrics aggregation.`.
====
